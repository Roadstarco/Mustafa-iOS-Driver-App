//
//  TripInternationalRequestsCell.swift
//  RoadStarRider
//
...
class TripInternationalRequestsCell: UITableViewCell {
    ...
    func setUp(request: InternationalRequest, parent: UIViewController) {
        
        vwMain.layer.cornerRadius = 12
        
        let date = request.createdAt ?? ""
        let id = request.bookingID ?? ""
        let earning = "$ \(request.tripAmount ?? 0)"
